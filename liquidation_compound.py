...
from configuration import (
    config_init, token_aggregator_mapping,
    get_reserves, get_signal_filters, 
    price_cache_read, price_cache_write, price_scale,
...
from get_users_from_logs import (
    ...
    get_exchange_rate, get_users_start, query_reserve_factor,
    get_borrow_index, query_collateral_factor,
...
def calculate_health_factor(usr, exchange_rates):
    ...
    for token_addr, reserve in usr[2].items():
        ...
        logger.debug("user {}, reserve {}, price {}, ctoken balance {}, borrow balance {}, exchange rate {}, reserve factor {}".
                    format(usr[0], token_addr, price, collateral_balance, debt_balance, exchange_rate, collateral_factor))
    ...
    if sum_borrow_plus_effects <= 0:
        ...
        logger.debug("user {} (0, liquidity {}, shortfall {}), health factor {}, sum collateral {}, sum borrow {}".
                     format(usr[0], liquidity, short_fall, health_factor, sum_collateral, sum_borrow_plus_effects))
...
def health_factor_and_liquidation(usr, block_num, exchange_rates, liq_incentive, q):
    if usr[3] is None or (0.9 <= usr[3][0]/HF_THRESHOLD <= 1.2):
        new_health_factor = calculate_health_factor(usr, exchange_rates)
        q.put((usr[0], new_health_factor), block=False)
    elif 0.7 <= usr[3][0]/HF_THRESHOLD < 0.9 or 1.2 < usr[3][0]/HF_THRESHOLD:
        f = abs(usr[3][0]/HF_THRESHOLD - 1)//0.1
        if usr[3][1] + min(100 * f, 1800) < block_num:
            new_health_factor = calculate_health_factor(usr, exchange_rates)
            q.put((usr[0], new_health_factor), block=False)
        else:
            return
    ...
    if 1 > new_health_factor/HF_THRESHOLD > 0.7:  # 1 ether
...
def pt(message, is_check, f):
    ...
    for r in res:
        ...
        new_price = price_scale(token_addr, r[1])
    ...
    logger.debug("time: {}, liquidation incentive: {}, query exchange rates: {}".
                 format(query_end - query_start, liquidation_incent, exchange_rates))
